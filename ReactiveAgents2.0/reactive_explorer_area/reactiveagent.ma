...
%*****************NEIGHBOR PORTS*******************%
neighborports : occ celltype explored comfort desire
...
%****************STATE VARIABLES*******************%
statevariables : agent cell heat pressure wetness desirability visibility
...
rule : { ~occ:= $agent + $cell;
		...
		~desire:=  #macro(DESIRABILITY); } {$visibility:= $cell; } 0  
							{ (0,0)~occ < 1
							and (0,0)~explored = 0
							and ( (0,-1)~occ >= 1 or (1,-1)~occ >= 1 or (-1,-1)~occ >= 1
								or (0,1)~occ >= 1 or (1,1)~occ >= 1  or (-1,1)~occ >= 1
								or (1,0)~occ >= 1 or (-1,0)~occ >= 1 )
							}
